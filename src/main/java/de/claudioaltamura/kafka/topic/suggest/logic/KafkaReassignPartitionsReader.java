...
public class KafkaReassignPartitionsReader {
  ...
  public KafkaTopicsDescribeDetails read(String topic, int paritionsCount, int replicationFactor,
      int[] partitionLeader) {
  ...
  private void assertPartitionCountEqualsPartitionLeaders(int paritionsCount,
      int[] partitionLeader) {

    if (partitionLeader.length != paritionsCount) {
      var errorMsg =
          String.format("partition leader length '%d' must be equal to partitions count '%d'.",
